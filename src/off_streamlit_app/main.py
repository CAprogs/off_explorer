...
def main() -> None:
    st.set_page_config(
        page_title="Food Search", page_icon=":stuffed_flatbread:", layout="wide"
    )
    ...
    with right_bottom:
        ...
        if right_column.button("Search Product", icon=":material/manage_search:"):
            st.session_state["image_url"], st.session_state["product_details"] = (
                find_image_by_barcode(st.session_state.get("user_input"))
            )
    ...
    with left_bottom:
        ...
        if st.session_state.get("image_url"):
            left_column.image(
                image=st.session_state["image_url"],
                caption=st.session_state["product_details"]["product_name"],
            )
            right_column.markdown(
                f"Product:  **{st.session_state['product_details']['product_name']}**"
            )
            right_column.markdown(
                f"Brand:  **{st.session_state['product_details']['brand']}**"
            )
            right_column.markdown(
                f"Quantity:  **{st.session_state['product_details']['quantity']}**"
            )
