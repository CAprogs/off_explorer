...
def test_value_isin_df(
    mock_df: pd.DataFrame, column: str, value: str, expected: bool
) -> None:
...
def test_fetch_product_details(
    mock_df: pd.DataFrame, barcode: str, expected: dict[str, str]
) -> None:
    assert fetch_product_details(mock_df, barcode) == expected


def test_process_barcode(
    capsys: pytest.CaptureFixture[str], mock_df: pd.DataFrame
) -> None:
    ...
    assert (
        "Barcode must be a 13 length digit value present in the dataset."
        in captured.out
    )
